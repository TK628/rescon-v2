...
function showSpectatorLoading(text){
    waitingScreen.style.display="flex";
    matchArea.style.display="none";
    waitingScreen.innerHTML=`
        <div class="spectator-loading">
            <div class="spectator-loading-title">RESCUE SYSTEM</div>
            <div class="spectator-loading-text">${text}</div>
            <div class="spectator-loading-line"></div>
        </div>
    `;
}
...
function showMissionStartOverlay(){
    if(missionStartShowing){
        return;
    }

    missionStartShowing=true;

    const overlay=document.createElement("div");
    overlay.className="spectator-start-overlay";
    overlay.innerHTML=`
        <div class="spectator-start-panel">
            <div class="spectator-start-small">MISSION</div>
            <div class="spectator-start-main">START</div>
        </div>
    `;
...
function updateDummyView(
    ...
    dummy1Bar.style.width=
        `${((hp1/(dummy1.maxHp??MAX_LIFE))*100)}%`;

    dummy2Bar.style.width=
        `${((hp2/(dummy2.maxHp??MAX_LIFE))*100)}%`;

    dummy3Bar.style.width=
        `${((hp3/(dummy3.maxHp??MAX_LIFE))*100)}%`;
    ...
    onValue(
        ...
        (
            ...
            if(roomData.finished){
                ...
                matchArea.style.display="none";
                waitingScreen.innerHTML=`
                    <div class="spectator-loading mission-complete-panel">
                        <div class="spectator-loading-title">MISSION COMPLETE</div>
                        <div class="spectator-loading-text">PLEASE WAIT FOR REFEREE RESULT</div>
                        <div class="spectator-loading-line"></div>
                    </div>
                `;
                return;
            ...
            if(
                ...
                showSpectatorLoading("WAITING FOR REFEREE...");
                return;

            }else if(!(roomData.timer||{}).endTime&&!(roomData.timer||{}).countdown){

                showSpectatorLoading("CHECKING NETWORK...");
                return;
            ...
            if(timer.countdown&&timer.countdownEndTime){
                ...
                ()=>{
                    ...
                    if(count>0){
                        waitingScreen.innerHTML=`
                            <div class="spectator-countdown">
                                <div class="spectator-countdown-label">MISSION STARTING</div>
                                <div class="spectator-countdown-number">${count}</div>
                                <div class="spectator-countdown-sub">RESCUE NETWORK READY</div>
                            </div>
                        `;
                    }else{
                        waitingScreen.innerHTML=`
                            <div class="spectator-countdown">
                                <div class="spectator-countdown-label">MISSION</div>
                                <div class="spectator-countdown-start">START</div>
                                <div class="spectator-countdown-sub">LIVE FEED OPENING</div>
                            </div>
                        `;
                    }
                };
